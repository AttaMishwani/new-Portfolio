...
const Projects = ({ projects }) => {
  return (
    <section id="projects" className="projects-section section-block">
      ...
      <div className="projects-wrap mt-12 flex min-h-[70vh] items-center overflow-hidden">
        <div className="projects-track flex gap-6 px-6">
          {projects.map((project) => (
            <article
              key={project.name}
              className="project-card relative min-w-[320px] flex-1 rounded-[32px] border border-white/10 bg-white/5 p-8 transition-all duration-300 hover:border-white/40 lg:min-w-[420px]"
            >
              <div className="absolute right-6 top-6 rounded-full border border-white/10 px-3 py-1 text-xs uppercase tracking-[0.3em] text-white/50">
                {project.role}
              </div>
              <h3 className="mt-12 font-display text-2xl uppercase tracking-[-0.01em] text-white">
                {project.name}
              </h3>
